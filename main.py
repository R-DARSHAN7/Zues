...
def cleanup_audio():
    cutoff = datetime.now(timezone.utc) - timedelta(minutes=settings.audio_ttl_minutes)
    deleted = 0
...
async def lifespan(app: FastAPI):
    await init_db()
    logger.info("NOVA online. Supabase connected.")
    scheduler = AsyncIOScheduler(timezone="UTC")
    scheduler.add_job(
        cleanup_audio, trigger="interval", minutes=settings.audio_ttl_minutes,
...
app = FastAPI(title="NOVA v2", lifespan=lifespan)
    ...
    try:
        while True:
            ...
            async with AsyncSessionLocal() as db:
                history = await get_recent(db, profile_id, limit=6)
...
async def _persist(profile_id: str, user_text: str, bot_text: str, domain: str):
    async with AsyncSessionLocal() as db:
